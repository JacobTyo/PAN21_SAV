...
from dsets.blog import get_blogs_train_or_test, get_training_for_triplet_loss_blogs, BlogsContrastiveDataset, BlogsOSContrastiveDataset, get_training_for_triplet_loss_blogsos, get_blogsos_contrastive_test
from dsets.fanfiction import get_ff_triplet_training, FFDataset, get_ff_contrastive_train_or_test
from utils.AaEvaluator import AaEvaluator, ContrastiveEvaluator, ContrastiveChunkerEvaluator
    ...
    with mlflow.start_run(run_name=args.run_name):
        ...
        if not args.model_path:
            ...
            if args.distributed:
                ...
                model = DistributedSentenceTransformer(modules=[word_embedding_model, pooling_model, dense_model],
                                                       device=args.device)
                model.load_state_dict(torch.load(args.model_path + '/state_dict.pt', map_location=args.device))
        ...
        # set up an evaluator
        margin = args.margin if args.which_loss_fn != 'ModifiedContrastiveLoss' else (args.margin + args.same_margin)/2
        if args.which_loss_fn == 'ContrastiveLoss' and args.which_dataset != 'imdb' and args.which_dataset != 'blogs' or args.which_dataset == 'blogsos' or args.which_dataset == 'fanfiction':
            if args.chunk_test:
                evaluator = ContrastiveChunkerEvaluator(test_dataset,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--chunk_test', action='store_true',
                        help='for use with threshold finder')
