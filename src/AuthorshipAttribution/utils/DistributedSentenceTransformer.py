...
class DistributedSentenceTransformer(nn.Sequential):
    """
    ...
    def start_multi_process_pool(self, target_devices: List[str] = None):
        ...
        for cuda_id in target_devices:
            p = ctx.Process(target=DistributedSentenceTransformer._encode_multi_process_worker, args=(cuda_id, self, input_queue, output_queue), daemon=True)
            p.start()
            processes.append(p)
    ...
    def save(self, path):
        ...
        # just save the state dict for now?
        # this would be nice to break into modules, that way I don't have to worry about dataparallel issues
        torch.save(self.state_dict(), path+'/state_dict.pt')

        # save the attributes without dataparallel for reuse on other machines
        # contained_modules = []
        #
        first = True
        for idx, name in enumerate(self._modules):
            ...
            # if first:
            #     torch.save(module.module.state_dict(), model_path + '/state_dict.pt')
            #     first = False
            # else:
            torch.save(module.state_dict(), model_path + '/state_dict.pt')
